...
export async function POST(req: Request) {
  ...
  const created = await db.feeStructure.create({
    data: {
      ...
      examFee:
        body.examFee != null
          ? new Prisma.Decimal(Number(body.examFee).toFixed(2))
          : null,
      transportFee:
        body.transportFee != null
          ? new Prisma.Decimal(Number(body.transportFee).toFixed(2))
          : null,
      miscFee:
        body.miscFee != null
          ? new Prisma.Decimal(Number(body.miscFee).toFixed(2))
          : null,
  ...
  // Auto-create FeePayment for all students
